...
ItemPtr eightNumber::init_problem(){
    ...
    if(head!=NULL){
        ...
        switch (mode_h) {
        case 1:
            head->h_x=count_h1(head);
            break;
        case 2:
            head->h_x=count_h2(head);
            break;
        case 3:
            head->h_x=count_h3(head);
            break;
        default:
            head->h_x=count_h1(head);
        }
...
ItemPtr eightNumber::deal()
{
    ...
    while(end&&open->head!=NULL){
        ...
        if(check(first)){  //检查当前open表的第一个节点是否为目标结点 1是目标结点，0则不是
            end=0;//找到目标结点
            break;
        }
        add_close(close,first); //将扩展节点加入close表，该节点扩展成功
        show_closed(close->tail);

        expand(open,close,first); //拓展该节点的下一个状态
        show_open(open->tail);
...
void eightNumber::step(int x,int y,int mark,ItemPtr node,DListPtr open,DListPtr close)
{
    ...
    if(new_node){
        ...
        switch (mode_h) {
        case 1:
            new_node->h_x=count_h1(new_node);
            break;
        case 2:
            new_node->h_x=count_h2(new_node);
            break;
        case 3:
            new_node->h_x=count_h3(new_node);
            break;
        default:
            new_node->h_x=count_h1(new_node);
        }
...
void eightNumber::expand(DListPtr open,DListPtr close,ItemPtr cur_node)
{
    ...
    position=find_0(cur_node); //找到0所在的格子 返回的是(行号+1)*10+列号; 即[行][列]
    position_x=position/10-1;  //得到列号
    position_y=position%10;  //得到行号
    give_try(position_x,position_y,cur_node,open,close); //根据空格（0）所在位置进行拓展试探，进行判重
}
...
//select 选择启发式函数 默认是h1
int eightNumber::select_H(){
    int mode_h = 1;
...
//动态显示open表
int eightNumber::show_open(ItemPtr node){
    if (node != NULL) {
        int i = show_open(node->pre);
...
//动态显示closed表
int eightNumber::show_closed(ItemPtr node){
    if (node != NULL) {
        int i = show_closed(node->pre);
